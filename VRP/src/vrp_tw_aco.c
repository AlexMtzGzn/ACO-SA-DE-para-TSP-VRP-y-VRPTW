...
void inicializar_hormiga(struct vrp_configuracion *vrp, struct individuo *ind, struct hormiga *hormiga)
{
    ...
    for (int i = 0; i < ind->numHormigas; i++)
    {
        ...
        for (int i = 0; i < ind->vehiculos; i++)
            ...
            inserta_vehiculo_flota(&hormiga[i], vrp, i + 1);
    }
}
...
{
    // Generar un número aleatorio entre 1 y vehiculos_necesarios
    int vehiculo_aleatorio = rand() % hormiga->vehiculos_necesarios + 1;

    // Accedemos a la flota de vehículos
    struct nodo_vehiculo *flota_vehiculo = hormiga->flota->cabeza;
    struct nodo_vehiculo *vehiculo_actual = flota_vehiculo;

    // Recorrer toda la lista de vehículos
    while (vehiculo_actual != NULL) // Se cambia la condición para recorrer toda la lista
    {
        struct vehiculo *vehiculo = vehiculo_actual->vehiculo;

        // Verificar si el id del vehículo coincide con el aleatorio generado
        if (vehiculo->id_vehiculo == vehiculo_aleatorio)
            return vehiculo;
...
void aco(struct vrp_configuracion *vrp, struct individuo *ind, struct hormiga *hormiga, double **instancia_visibilidad, double **instancia_feromona, double **instancia_distancias, double **instancia_ventanas_tiempo)
{
    ...
    while (hormiga->tabu_contador < vrp->num_clientes)
    {
        ...
        vehiculo = seleccionamos_vehiculo_aleatoriemente(hormiga);
    ...
    for (int i = 0; i < hormiga->vehiculos_necesarios; i++)
