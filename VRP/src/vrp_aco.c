...
void inicializar_hormiga(struct vrp_configuracion *vrp, struct individuo *ind, struct hormiga *hormiga)
{
    ...
    for (int i = 0; i < ind->numHormigas; i++)
    {
        ...
        hormiga[i].vehiculos_necesarios = ind->vehiculos;
        ...
        for (int j = 0; j < hormiga[i].vehiculos_necesarios; j++)
            inserta_vehiculo_flota(&hormiga[i], vrp, j + 1);
    }
}

void calcular_posibles_clientes(int origen, struct vrp_configuracion *vrp, struct hormiga *hormiga)
{
...
{
    if (hormiga == NULL || hormiga->flota == NULL || hormiga->flota->cabeza == NULL)
        return NULL;

    // Elegimos una posición aleatoria entre 1 y el número de vehículos necesarios
    int posicion = (rand() % hormiga->vehiculos_necesarios);

    struct nodo_vehiculo *nodo_actual = hormiga->flota->cabeza;

    // Avanzamos hasta la posición deseada (0-based index)
    for (int i = 0; i < posicion && nodo_actual != NULL; i++)
        nodo_actual = nodo_actual->siguiente;

    return nodo_actual ? nodo_actual->vehiculo : NULL;
}
...
void aco(struct vrp_configuracion *vrp, struct individuo *ind, struct hormiga *hormiga, double **instancia_visibilidad, double **instancia_feromona, double **instancia_distancias)
{
    ...
    while (hormiga->tabu_contador < vrp->num_clientes)
    {
        ...
        calcular_posibles_clientes(origen, vrp, hormiga);

        if (hormiga->posibles_clientes_contador == 0)
            break;

        // Seleccionamos un vehículo aleatoriamente
        vehiculo = seleccionamos_vehiculo_aleatoriamente(hormiga);

        // Obtenemos el último cliente de la ruta del vehículo
        ruta = vehiculo->ruta;
        ultimo_cliente_ruta = ruta->cola;
        origen = ultimo_cliente_ruta->cliente;  // Actualizamos el origen al último cliente visitado
...
void vrp_aco(struct vrp_configuracion *vrp, struct individuo *ind, double **instancia_visiblidad, double **instancia_distancias, double **instancia_feromona)
{
    ...
    for (int i = 0; i < ind->numIteraciones; i++)
    {
        ...
        if (i < ind->numIteraciones - 1)
        {
            for (int j = 0; j < ind->numHormigas; j++)
            {
                reiniciar_hormiga(&hormiga[j], ind, vrp);
            }
        }
    }
