...
void actualizar_feromona(struct individuo *ind, struct vrp_configuracion *vrp, struct hormiga *hormiga, double **instancia_feromona)
{
   ...
   // Liberar memoria
   liberar_instancia(delta_feromona, vrp->num_clientes);
}
...
{
   int contador = 0; // Declaramos y inicializamos contador
   for (int i = 0; i < vrp->num_clientes; i++)
   {
      if (hormiga->tabu[i] == 0) // Si la posicion del arreglo es igual a 0 entocnes incrementa el contador
         contador++;
   }
   return contador;
}

void calcular_fitness_hormiga(struct vrp_configuracion *vrp, struct hormiga *hormiga)
{
