...
\begin{titlepage}
	...
	\begin{minipage}[c][9.5cm][s]{10cm}
		\begin{center}
			...
			Departamento de Matemáticas Aplicadas y Sistemas\\[13pt]
...
\begin{tabularx}{\textwidth}{>{\bfseries}l X}
	...
	DE    & \textbf{D}ifferential \textbf{E}volution (\textit{Evolución Diferencial})                                                                               \\
	EA    & \textbf{E}volutionary \textbf{A}lgorithm (\textit{Algoritmo Evolutivo})                                                                                 \\
	...
	TSP   & \textbf{T}ravelling \textbf{S}alesman \textbf{P}roblem (\textit{Problema del Viajante})                                                                 \\
...
\subsection{Calibración de parámetros mediante Evolucion Diferencial (DE)}
...
\subsection{Construcción de rutas iniciales mediante Optimización por Colonia de Hormigas (ACO)}
...
\end{equation}
\noindent
\textbf{Ecuación \ref{eq:tiempo_llegada}:} Calcula el tiempo de llegada del vehículo al cliente $j$, sumando el tiempo de salida del cliente anterior $i$ y el tiempo de viaje desde $i$ hasta $j$.  
...
\end{equation}
\noindent
\textbf{Ecuación \ref{eq:inicio_servicio}:} Determina el instante en que inicia el servicio en el cliente $j$. Si el vehículo llega antes de $e_j$, debe esperar hasta el inicio de la ventana; si llega dentro de la ventana, comienza el servicio inmediatamente.  
...
\end{equation}
\noindent
\textbf{Ecuación \ref{eq:salida_cliente}:} Calcula el tiempo de salida del vehículo del cliente $j$, sumando al inicio del servicio el tiempo de servicio requerido $s_j$.

\subsubsection*{Mecanismo de gestión de flota}
...
\subsubsection*{Proceso de construcción de soluciones}

El algoritmo \ref{fig:algoritmo_construccion_solucion} presenta el proceso completo de construcción de una solución por parte de una hormiga, integrando la selección probabilística de clientes, la gestión de restricciones y la asignación dinámica de vehículos.
...
\begin{figure}[H]
	...
	\end{algorithm}
	\caption{Proceso de construcción probabilística de una solución completa por una hormiga. El algoritmo maneja dinámicamente la asignación de vehículos y el avance temporal cuando no hay clientes factibles disponibles, garantizando el cumplimiento de todas las restricciones del VRP-TW.}
	\label{fig:algoritmo_construccion_solucion}
\end{figure}

\subsubsection*{Evaluación de la función objetivo}

Una vez construida la solución, se evalúa su calidad calculando la distancia total recorrida por todos los vehículos. El algoritmo \ref{fig:algoritmo_evaluacion_fitness} describe este proceso de evaluación.

\begin{figure}[H]
	\centering
	\begin{algorithm}[H]
		\caption{Evaluación de Fitness de una Solución}
	...
	\end{algorithm}
	\caption{Cálculo del fitness de una solución como la suma de las distancias totales recorridas por todos los vehículos. Cada ruta se evalúa sumando las distancias entre clientes consecutivos, incluyendo los trayectos desde y hacia el depósito.}
...
Las mejores hormigas (determinadas por un porcentaje $p_{elite}$ de la población ordenada por fitness) depositan feromona en las aristas que utilizaron:
...
\begin{equation}
	\Delta\tau_{ij}^k = \begin{cases}
		\frac{1}{L_k} & \text{si la hormiga } k \text{ usó la arista } (i,j) \\
		0             & \text{en otro caso}
	\end{cases}
\end{equation}
...
Adicionalmente, se aplica un refuerzo extra a la mejor solución global encontrada hasta el momento, multiplicando su contribución de feromona por un factor de 2:
...
El algoritmo \ref{fig:algoritmo_actualizacion_feromonas} detalla el proceso completo de actualización de la matriz de feromonas.

\begin{figure}[H]
	\centering
	\begin{algorithm}[H]
		\caption{Actualización de Matriz de Feromonas}
		\label{alg:actualizacion_feromonas}
		\SetAlgoLined
		\SetAlFnt{\footnotesize}

		\KwIn{Matriz $\tau$, conjunto Elite de hormigas, MejorSoluciónGlobal, tasa $\rho$}
		\KwOut{Matriz $\tau$ actualizada}

		\textbf{Fase 1: Evaporación}\\
		\textbf{para} $i=0$ \textbf{hasta} $n-1$ \textbf{hacer}\\
		\hspace{1em} \textbf{para} $j=0$ \textbf{hasta} $n-1$ \textbf{hacer}\\
		\hspace{2em} \textbf{si} $i \neq j$ \textbf{entonces}\\
		\hspace{3em} $\tau_{ij} \gets (1 - \rho) \cdot \tau_{ij}$.\\
		\hspace{2em} \textbf{fin si}\\
		\hspace{1em} \textbf{fin para}\\
		\textbf{fin para}\\

		\textbf{Fase 2: Depósito por hormigas elite}\\
		\textbf{para cada} hormiga $k \in$ Elite \textbf{hacer}\\
		\hspace{1em} $\Delta \tau \gets 1 / \text{Costo}_k$.\\
		\hspace{1em} \textbf{para cada} vehículo $v$ en Solución$_k$ \textbf{hacer}\\
		\hspace{2em} \textbf{para cada} arista $(i,j)$ en ruta de $v$ \textbf{hacer}\\
		\hspace{3em} \textbf{si} $i \neq j$ \textbf{entonces}\\
		\hspace{4em} $\tau_{ij} \gets \tau_{ij} + \Delta\tau$.\\
		\hspace{4em} $\tau_{ji} \gets \tau_{ji} + \Delta\tau$ \textit{(grafo no dirigido)}.\\
		\hspace{3em} \textbf{si no}\\
		\hspace{4em} $\tau_{ii} \gets 0$ \textit{(eliminar autoenlaces)}.\\
		\hspace{3em} \textbf{fin si}\\
		\hspace{2em} \textbf{fin para}\\
		\hspace{1em} \textbf{fin para}\\
		\textbf{fin para}\\

		\textbf{Fase 3: Refuerzo elitista}\\
		$\Delta \tau_{best} \gets 2 / \text{CostoMejorGlobal}$.\\
		\textbf{para cada} vehículo $v$ en MejorSoluciónGlobal \textbf{hacer}\\
		\hspace{1em} \textbf{para cada} arista $(i,j)$ en ruta de $v$ \textbf{hacer}\\
		\hspace{2em} $\tau_{ij} \gets \tau_{ij} + \Delta\tau_{best}$.\\
		\hspace{2em} $\tau_{ji} \gets \tau_{ji} + \Delta\tau_{best}$.\\
		\hspace{1em} \textbf{fin para}\\
		\textbf{fin para}\\
	...
	\end{algorithm}
	\caption{Mecanismo de actualización de feromonas en tres fases: evaporación uniforme para favorecer exploración, depósito proporcional al fitness de las hormigas elite, y refuerzo adicional en la mejor solución global para intensificar la explotación de rutas prometedoras.}
...
\subsubsection*{Algoritmo principal ACO integrado}

El algoritmo \ref{fig:algoritmo_aco_principal} presenta la estructura completa del algoritmo ACO, integrando todas las fases descritas previamente: construcción de soluciones, evaluación, ordenamiento, aplicación de Simulated Annealing a las mejores hormigas, y actualización de feromonas.
...
\begin{figure}[H]
	...
	\begin{algorithm}[H]
		...
		\KwIn{Parámetros $\alpha, \beta, \gamma, \rho$, número de hormigas $m$, iteraciones $n_{iter}$, porcentaje elite $p_{elite}$}
		\KwOut{Mejor solución de rutas encontrada}

		\textbf{Inicialización:}\\
		Inicializar matriz de feromonas $\tau_{ij} = 1/l_j$ para todo $i, j$.\\
		Calcular matriz de visibilidad $\eta_{ij} = 1/d_{ij}$.\\
		Calcular matriz de información temporal $\omega_{ij} = 1/l_j$.\\
		MejorSoluciónGlobal $\gets$ NULL, MejorCostoGlobal $\gets \infty$.\\

		\textbf{Iteración principal:}\\
		...
		\hspace{2em} Solución$_k$ $\gets$ AplicarSimulatedAnnealing(Solución$_k$).\\
		\hspace{2em} Costo$_k$ $\gets$ EvaluarFitness(Solución$_k$).\\
...
\subsubsection*{Estructura de datos y gestión de memoria}

La implementación utiliza una estructura jerárquica eficiente:

\begin{itemize}
	\item \textbf{Hormiga:} Representa una solución completa con atributos de fitness global, número de vehículos necesarios y una lista enlazada de vehículos (flota).

	\item \textbf{Vehículo:} Contiene una ruta (lista enlazada de clientes), capacidad acumulada, tiempos de salida/llegada, velocidad y fitness individual.

	\item \textbf{Ruta:} Lista doblemente enlazada de nodos que representan clientes, permitiendo inserciones y eliminaciones eficientes en cualquier posición.

	\item \textbf{Gestión de memoria:} En cada iteración se realiza:
	      \begin{itemize}
		      \item Copia profunda de la solución actual para generar solución vecina
		      \item Liberación explícita de memoria al aceptar/rechazar movimientos
		      \item Protección contra fugas de memoria mediante funciones especializadas
	      \end{itemize}
\end{itemize}
