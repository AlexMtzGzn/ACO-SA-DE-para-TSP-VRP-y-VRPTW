...
Se realizaron experimentos utilizando instancias de referencia del conjunto Solomon, específicamente la familia C1, ampliamente utilizada en la literatura para evaluar algoritmos de ruteo con restricciones temporales. Los resultados muestran que el método híbrido propuesto es capaz de generar soluciones competitivas en términos de distancia total recorrida y número de vehículos utilizados. El desempeño obtenido se analiza y contextualiza con base en resultados reportados en la literatura.
\vspace{1em}

En conjunto, los resultados indican que la integración DE–ACO–SA logra un equilibrio adecuado entre exploración global, construcción guiada de rutas y mejora local intensiva, mostrando potencial para aplicaciones logísticas reales y abriendo la puerta a futuras mejoras orientadas a paralelización, incorporación de nuevas restricciones y análisis comparativo formal.
\vspace{1em}
...
The Vehicle Routing Problem with Time Windows (VRP-TW) is one of the most widely studied variants of the VRP due to its complexity and its applications in logistics, transportation, and distribution. Its NP-hard nature prevents medium and large instances from being solved through exact methods, which has motivated the development of heuristic and metaheuristic approaches capable of obtaining high-quality solutions within reasonable computational times.
\vspace{1em}

This work proposes a hybrid approach that integrates three complementary metaheuristic techniques: Differential Evolution (DE), Ant Colony Optimization (ACO), and Simulated Annealing (SA). The purpose of DE is to automatically calibrate the main parameters of the ACO and SA algorithms to improve exploration and exploitation of the search space according to the characteristics of each instance. ACO is then used to construct feasible initial routes under capacity and time-window constraints. Finally, SA serves as a local refinement method, improving solution quality through adaptive neighborhood operators.
\vspace{1em}

Experiments were conducted using benchmark instances from the Solomon dataset, specifically the C1 family, which is widely used in the literature to evaluate routing algorithms under time-window constraints. The results show that the proposed hybrid method is capable of generating competitive solutions in terms of total distance traveled and number of vehicles used. The performance obtained is analyzed and contextualized based on results reported in the literature.
\vspace{1em}

Overall, the results indicate that the integration of DE–ACO–SA achieves a suitable balance between global exploration, guided route construction, and intensive local improvement. This demonstrates the potential of the proposed method for real-world logistics applications and suggests future research directions involving parallelization, the incorporation of additional constraints, and formal comparative analyses.
\vspace{1em}
...
El presente estudio propuso un enfoque híbrido basado en la combinación de Evolución Diferencial (DE), Optimización por Colonia de Hormigas (ACO) y Recocido Simulado (SA) para resolver el Problema de Ruteo de Vehículos con Ventanas de Tiempo (VRP-TW). Los resultados obtenidos demostraron que la integración de estas tres técnicas permite construir soluciones factibles de manera consistente y posteriormente refinarlas para reducir la distancia total recorrida y el número de vehículos empleados. El método híbrido mostró ser competitivo frente a valores de referencia reportados para las instancias Solomon.

El objetivo principal del trabajo fue diseñar y evaluar un método híbrido capaz de mejorar el desempeño de técnicas metaheurísticas tradicionales aplicadas al VRP-TW. Este objetivo se cumplió satisfactoriamente al integrar DE para la calibración automática de parámetros, ACO para la construcción inicial de rutas y SA como mecanismo de mejora local. Cada componente cumplió su rol esperado dentro del enfoque híbrido, contribuyendo a obtener soluciones de mayor calidad y reduciendo el esfuerzo manual de calibración de parámetros.

El enfoque híbrido propuesto aporta evidencia de que la combinación ordenada de técnicas metaheurísticas puede superar el desempeño de sus versiones individuales. Además, la automatización de la calibración reduce significativamente la dependencia de conocimiento experto, lo cual abre la puerta a sistemas más autónomos para problemas complejos de logística. El método es flexible y puede adaptarse para incluir distintos operadores, restricciones adicionales o incluso otras metaheurísticas, lo que amplía su potencial de aplicación en diversos contextos prácticos.

Entre las líneas futuras de investigación se propone explorar versiones paralelas del método híbrido que permitan reducir tiempos de ejecución manteniendo la calidad de las soluciones. También resulta prometedor incorporar nuevas restricciones del mundo real, como ventanas de tiempo variables, demandas estocásticas o múltiples depósitos. Otra línea importante consiste en realizar comparaciones formales contra métodos híbridos del estado del arte y extender el análisis a otros conjuntos de instancias más desafiantes, así como evaluar el desempeño del método en problemas de mayor escala que reflejen mejor las complejidades de la logística moderna.
