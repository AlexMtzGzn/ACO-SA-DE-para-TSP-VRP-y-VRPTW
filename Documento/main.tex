...
\hypersetup{
    ...
    citecolor=black,
    urlcolor=black,
    pdftitle={VRP-TW: Proyecto Terminal},
...
\newenvironment{demo}{\par\noindent\textbf{Demostración:}\quad}{\hfill$\square$\par}
...
\begin{flushleft}
	...
	% Quisiera comenzar agradeciendo a Dios, por ser mi protector y guia en cada etapa de mi vida académica. Su presencia me ha dado sabiduría para superar los retos y alcanzar mis metas.
	...
	% A mi madre, Laura Guzman Carpip por creer siempre en mi, por la educaicon que me da, su amor condiconal , al igual que su apoyo, si ha alguien le debo este gran logro academi a es ella porque me mostro que esstudiar me sacaria delante  Su inspiración para esforzarme y alcanzar mis metas.

	% A mi padre, Alejandro Martinez Vazquez qwue siempre me me aconsejo de la mejor mejor, quoien desde pequeño me mostraba lo bueno y lo malo del mundo, su amor a pesar mis errores  y su enseñanzas fueron cavle para confiar en mis capacidades.

	% A mi hermana Linda, por su apoyo, compañía y amor incondicional atrvez de mi vida, siempre motivándome y acompañándome en cada etapa de mi vida. \\

	% A mis abuelos maternos, Margarita y Salvador, gracias por consentirme,  amarme brindarme cariño y enseñanzas que han marcado mi vida desde pequeño. \\

	% A mis abuelos paternos, Flor y Trinidad, gracias por su apoyo, amor, ejemplo y cariño constante, que me han acompañado en cada paso de mi formación y crecimiento personal. \\

	% A Alpha por su amor incondicional ,apoyo y amarme a pesar de mi carcater,  porque apessar de mis desperaciones 
...
El Problema de Ruteo de Vehículos con Ventanas de Tiempo (VRP-TW) es una de las variantes más estudiadas del VRP debido a su complejidad y a sus aplicaciones en logística, transporte y distribución. Su naturaleza NP-Difícil impide resolver instancias de tamaño medio o grande mediante técnicas exactas, lo que ha motivado el desarrollo de enfoques heurísticos y metaheurísticos capaces de obtener soluciones de alta calidad en tiempos razonables.
\vspace{1em}

En este trabajo se propone un enfoque híbrido que integra tres técnicas metaheurísticas complementarias: Evolución Diferencial (DE), Optimización por Colonia de Hormigas (ACO) y Recocido Simulado (SA). El objetivo del DE es calibrar automáticamente los parámetros principales de los algoritmos ACO y SA, con el fin de mejorar la exploración y explotación del espacio de búsqueda según el tamaño de cada instancia. Posteriormente, ACO se emplea para construir rutas iniciales factibles bajo restricciones de capacidad y ventanas de tiempo. Finalmente, SA funciona como método de refinamiento local para mejorar la calidad de las soluciones mediante operadores de vecindad adaptativos.
\vspace{1em}

Se realizaron experimentos utilizando instancias de referencia del conjunto Solomon, ampliamente utilizada en la literatura para evaluar algoritmos de ruteo con restricciones temporales. Los resultados muestran que el método híbrido propuesto es capaz de generar soluciones competitivas en términos de distancia total recorrida y número de vehículos utilizados. El desempeño obtenido se analiza y contextualiza con base en resultados reportados en la literatura.
\vspace{1em}

En conjunto, los resultados indican que la integración DE–ACO–SA logra un equilibrio adecuado entre exploración global, construcción guiada de rutas y mejora local intensiva, mostrando potencial para aplicaciones logísticas reales y abriendo la puerta a futuras mejoras orientadas a paralelización y incorporación de nuevas restricciones.
\vspace{1em}
...
The Vehicle Routing Problem with Time Windows (VRP-TW) is one of the most extensively studied variants of the VRP due to its complexity and its applications in logistics, transportation, and distribution. Its NP-hard nature prevents medium- or large-scale instances from being solved through exact methods, which has motivated the development of heuristic and metaheuristic approaches capable of obtaining high-quality solutions within reasonable computational times.
\vspace{1em}

In this work, a hybrid approach integrating three complementary metaheuristic techniques is proposed: Differential Evolution (DE), Ant Colony Optimization (ACO), and Simulated Annealing (SA). The objective of DE is to automatically calibrate the main parameters of the ACO and SA algorithms, in order to improve the exploration and exploitation of the search space according to the size of each instance. Subsequently, ACO is employed to construct feasible initial routes under capacity and time-window constraints. Finally, SA serves as a local refinement method to improve solution quality through adaptive neighborhood operators.
\vspace{1em}

Experiments were conducted using reference instances from the Solomon dataset, widely used in the literature to evaluate routing algorithms with temporal constraints. The results show that the proposed hybrid method is capable of generating competitive solutions in terms of total distance traveled and number of vehicles used. The performance obtained is analyzed and contextualized based on results reported in previous studies.
\vspace{1em}

Overall, the results indicate that the DE–ACO–SA integration achieves an adequate balance between global exploration, guided route construction, and intensive local improvement, showing potential for real-world logistics applications and paving the way for future enhancements oriented toward parallelization and the incorporation of additional constraints.
\vspace{1em}
...
% Ajustes para tocloft
\cftsetindents{subsection}{2em}{3em}
\cftsetindents{subsubsection}{4em}{3em}

% Profundidad de índice y numeración
...
% --- Índice de fórmulas (ajuste de ancho de número + lista) ---
\cleardoublepage
\phantomsection
\addcontentsline{toc}{chapter}{Índice de Fórmulas}
\setlength{\cftmyequationsnumwidth}{2.5em} % ancho para números en lista de ecuaciones
\listofmyequations
...
Diseñar, implementar, probar y evaluar un algoritmo híbrido que combine la ACO, el SA  y el DE, con el objetivo de resolver de manera eficiente el VRP-TW.
...
\begin{enumerate}
	...
	\item Evaluar el rendimiento del algoritmo híbrido en términos de calidad de la solución y eficiencia computacional, utilizando instancias de prueba estándar aceptadas en la literatura, y comparando su desempeño con los resultados documentados en trabajos relacionados.
\end{enumerate}
...
El estudio de problemas de optimización en rutas se remonta al desarrollo del Problema del Agente Viajero, formulado a principios del siglo XX, cuyo objetivo es encontrar la ruta de costo mínimo que recorra un conjunto de ciudades exactamente una vez, regresando al punto de partida. A lo largo de las décadas, este problema se ha consolidado como uno de los pilares de la optimización combinatoria \citep{papadimitriou1994}.

Más adelante, se propuso el Problema de Ruteo de Vehículos como una generalización del TSP, considerando una flota de vehículos que deben atender a múltiples clientes desde un depósito central. Este problema fue introducido por primera vez por Dantzig y Ramser en 1959 para optimizar rutas de distribución de gasolina \citep{dantzig1959truck}.
...
\begin{itemize}
	\item \textbf{Algoritmos Genéticos}, inspirados en la evolución natural, propuestos por Holland en 1975, que utilizan operadores genéticos para evolucionar una población de soluciones \citep{holland1975adaptation}.
	\item \textbf{Algoritmo de Colonias de Hormigas}, propuesto inicialmente por Dorigo en 1992, basado en el comportamiento colectivo de las hormigas para resolver problemas combinatorios \citep{dorigo1992}.
	\item \textbf{Recocido Simulado}, inspirado en el proceso físico de enfriamiento de metales, introducido por Kirkpatrick, Gelatt y Vecchi en 1983, que permite escapar de óptimos locales mediante la aceptación controlada de soluciones peores \citep{kirkpatrick1983}.
	\item \textbf{Búsqueda Tabú}, una técnica heurística que explora iterativamente soluciones vecinas para mejorar la calidad de la solución, evitando ciclos mediante la prohibición temporal de movimientos \citep{aarts1988local}.
\end{itemize}
...
En este trabajo, el problema abordado —VRP-TW— se considera un problema decidible. A continuación, se presenta una clasificación más detallada dentro de esta categoría, basada en la teoría de la complejidad computacional.
...
El crecimiento exponencial del tiempo requerido para resolver el problema VRP-TW lo ubica dentro de la clase NP. Este problema, aunque decidible, presenta una complejidad tan elevada que no se conoce ningún algoritmo exacto que lo resuelva eficientemente para instancias grandes. Por ello, más adelante se analizará su clasificación específica dentro de NP.
...
Los problemas de optimización combinatoria son ampliamente utilizados en planificación y enrutamiento de vehículos. Entre ellos destaca el TSP como base de los modelos de rutas y sus variantes con restricciones más realistas, como capacidad o ventanas de tiempo. En este ámbito, el VRP y el VRP-TW son dos de los casos más estudiados por su relevancia práctica y alta complejidad, por lo que a continuación se describen con detalle.
...
\begin{figure}[H]
	...
	\caption{Ejemplificación de una Solución al Problema del Agente Viajero.}
	\label{fig:tsp}
\end{figure}
...
\subsubsection*{Modelo Matemático del Problema del Agente Viajero}
...
\begin{equation}
	...
	\label{eq:TSP_out}
\end{equation}
\addcontentsline{eq}{myequations}{\protect\numberline{\theequation}Restricción: salida única de cada nodo}

\begin{equation}
	\sum_{i=1}^N x_{ij} = 1, \quad \forall j = 1,\dots,N
	\label{eq:TSP_in}
\end{equation}
\addcontentsline{eq}{myequations}{\protect\numberline{\theequation}Restricción: entrada única a cada nodo}

\begin{equation}
	u_i - u_j + N x_{ij} \leq N - 1, \quad \forall i,j = 2,\dots,N, \; i \neq j
	\label{eq:TSP_subtour}
\end{equation}
\addcontentsline{eq}{myequations}{\protect\numberline{\theequation}Restricción: eliminación de subciclos}
...
\subsubsection*{Complejidad del Problema del Agente Viajero}

Como explica \cite{papadimitriou1998}, el TSP es NP-Difícil, y su versión de decisión es NP-completa. Esto implica que la búsqueda de un algoritmo eficiente para resolverlo representa un desafío fundamental en la teoría de la computación, pues encontrar una solución polinomial para el TSP permitiría resolver eficientemente toda la clase \textit{NP}.
...
\subsubsection{Problema del Problema Ruteo de Vehículos}
...
\begin{figure}[H]
	...
	\caption{Ejemplificación de una Solución al Problema de Ruteo de Vehículos.}
	\label{fig:vrp}
\end{figure}
...
\subsubsection*{Modelo Matemático del Problema de Ruteo de Vehículos}
...
\subsubsection*{Complejidad del Problema de Ruteo de Vehículos}
\label{subsec:complejidad_vrp}

Como señalan Laporte y Nobert \cite{laporte1987}, el VRP es NP-Difícil, incluso en su versión más simple con un solo vehículo (es decir, el TSP).

El VRP representa la extensión natural del TSP hacia escenarios logísticos más realistas y complejos, incorporando restricciones operativas que reflejan las limitaciones del mundo real. Su estudio es fundamental para comprender cómo las restricciones de capacidad, múltiples vehículos y la gestión de flotas impactan en la optimización de rutas.
\subsubsection{Problema de Ruteo de Vehículos con Ventanas de Tiempo}
...
\begin{figure}[H]
	...
	\caption{Ejemplificación de una Solución al Problema de Ruteo de Vehículos con Ventanas de Tiempo.}
	\label{fig:vrptw}
\end{figure}
...
\subsubsection*{Modelo Matemático del Problema de Ruteo de Vehículos con Ventanas de Tiempo}

\begin{equation}
	\min \sum_{k \in K} \sum_{i \in V} \sum_{j \in V} c_{ij} x_{ijk}
	\label{eq:VRPTW_obj}
\end{equation}
\addcontentsline{eq}{myequations}{\protect\numberline{\theequation} Función objetivo VRP-TW: minimizar el costo total del recorrido}
...
\subsubsection*{Complejidad del Problema de Ruteo de Vehículos con Ventanas de Tiempo}
...
Por ello, resulta fundamental comprender los fundamentos del TSP y del VRP, ya que constituyen la base teórica sobre la cual se desarrolla el VRP-TW, que es el problema principal abordado en este trabajo.

\subsection{Métodos de Solución para el Problema de Ruteo de Vehículos con Ventanas de Tiempo}
Resumiendo las secciones pasadas, los problemas de enrutamiento como el TSP, el VRP y su extensión con ventanas de tiempo, el VRP-TW, mencionados anteriormente, pertenecen a la clase de problemas NP-Difíciles \citep{papadimitriou1998, laporte1987, toth2014}. Esta clasificación se debe al crecimiento exponencial del tiempo requerido para resolverlos a medida que aumenta el número de nodos o clientes en la instancia.

Dada su complejidad computacional, el problema VRP-TW ha sido abordado mediante diversas estrategias de solución que tienen como objetivo minimizar el costo total del recorrido, respetando las restricciones impuestas. Entre ellas se incluyen los métodos exactos, así como las técnicas heurísticas y metaheurísticas.

A continuación, se describen estos enfoques, junto con ejemplos representativos aplicados al problema del VRP-TW.
...
En relación con lo expuesto en la subsubsection~\ref{subsec: clase_np}, los problemas de decisión pertenecientes a la clase NP presentan una complejidad tal que no es posible garantizar la obtención de una solución óptima en tiempo polinómico razonable; en este contexto, los métodos heurísticos constituyen una alternativa eficaz para abordar el problema de optimización combinatoria VRP-TW, donde los métodos exactos resultan inviables a gran escala. Por lo tanto, las heurísticas permiten obtener soluciones satisfactorias en tiempos razonables, priorizando el equilibrio entre la calidad de la solución y la eficiencia computacional; aunque no garantizan la optimalidad, su aplicación resulta esencial cuando las soluciones exactas son prohibitivas.
...
Aunque los algoritmos evolutivos y los algoritmos genéticos han sido descritos de manera general, a continuación se presenta una variante de especial relevancia para esta investigación. Dicha técnica es fundamental en el desarrollo del presente trabajo, ya que se implementó como parte del enfoque de optimización propuesto.
...
El DE comparte los fundamentos biológicos de otras metaheurísticas evolutivas, como la selección natural y la supervivencia del más apto \cite{back1996evolutionary}, pero se diferencia por su mecanismo basado en combinaciones vectoriales de diferencias entre individuos de la población, lo que le permite mantener un equilibrio adecuado entre exploración y explotación \cite{das2011differential}.

Este algoritmo trabaja con una población de soluciones candidatas representadas como vectores de valores reales, que evolucionan a lo largo de varias generaciones mediante tres operadores principales: mutación, recombinación y selección \cite{price2005differential}. De manera general, su funcionamiento puede describirse de la siguiente forma:

\begin{itemize}
	\item \textbf{Mutación:} genera un vector mutante $v_i$ combinando individuos de la población \cite{storn1997differential}. Una de las estrategias más utilizadas es DE/rand/1:
	      \begin{equation}
		      v_i = x_{r_1} + F \cdot (x_{r_2} - x_{r_3})
	      \end{equation}
	      \addcontentsline{eq}{myequations}{\protect\numberline{\theequation} Vector mutante en Evolución Diferencial (DE/rand/1)}
	      donde $r_1, r_2, r_3$ son índices aleatorios distintos entre sí y diferentes de $i$, y $F$ es el factor de mutación ($F \in [0, 2]$, típicamente entre 0.5 y 1) \cite{price2005differential,das2011differential}. Este operador introduce diversidad y permite explorar nuevas regiones del espacio de búsqueda.

	\item \textbf{Recombinación:} combina el vector mutante con el vector original $x_i$ para formar un vector de prueba $u_i$, siguiendo el esquema de cruce binomial \cite{storn1997differential}:
	      \begin{equation}
		      u_{i,j} =
		      \begin{cases}
	      ...
	      donde $CR \in [0, 1]$ es la tasa de cruce o probabilidad de recombinación \cite{price2005differential}. Este operador combina información de distintas soluciones, favoreciendo la explotación de configuraciones prometedoras.

	\item \textbf{Selección:} compara el vector de prueba $u_i$ con el individuo original $x_i$ mediante la función objetivo, conservando únicamente el de mejor aptitud para la siguiente generación \cite{storn1997differential}:
	      \begin{equation}
		      x_i^{(t+1)} =
		      \begin{cases}
	      ...
	      Este mecanismo garantiza la mejora progresiva de la población, evitando el deterioro de soluciones de alta calidad \cite{price2005differential}.
\end{itemize}
...
La Optimización por Colonias de Hormigas (\textit{Ant Colony Optimization},ACO) es una metaheurística bio-inspirada que toma como referencia el comportamiento colectivo de las hormigas al buscar caminos eficientes entre su colonia y una fuente de alimento \citep{dorigo1997ant}.
...
\end{equation}
\addcontentsline{eq}{myequations}{\protect\numberline{\theequation} Regla de transición en Optimización por Colonia de Hormigas}
...
Cada una de las metaheurísticas mencionadas —Evolución Diferencial, Optimización por Colonias de Hormigas y Recocido Simulado— puede aplicarse eficazmente a problemas de optimización complejos. Estas técnicas han demostrado ser herramientas valiosas en la resolución del VRP-TW, gracias a su capacidad para explorar amplios espacios de búsqueda y aproximarse a soluciones óptimas en tiempos computacionales razonables.

Si bien existen numerosas estrategias metaheurísticas documentadas en la literatura, en este trabajo se han seleccionado estas tres por su relevancia teórica, versatilidad y desempeño comprobado en problemas de optimización combinatoria.
...
\begin{itemize}
	...
	\item \textbf{DE + Recocido Simulado:} integra Evolución Diferencial con Recocido Simulado, equilibrando exploración global y ajuste fino de soluciones.
...
\noindent En conjunto, estas instancias permiten evaluar el desempeño, eficiencia y calidad de las soluciones generadas por los algoritmos para distintos tamaños y configuraciones del problema VRP-TW.
...
Para resolver el Problema de Enrutamiento de Vehículos con Ventanas de Tiempo, se propone un algoritmo híbrido que combina tres técnicas metaheurísticas: el Algoritmo Evolutivo Diferencial, la Optimización por Colonia de Hormigas y el Recocido Simulado.
...
\begin{enumerate}
	\item \textbf{Algoritmo Evolutivo Diferencial:} se encarga de calibrar los parámetros de ACO y SA para optimizar la búsqueda de soluciones.
	\item \textbf{Optimización por Colonia de Hormigas:} genera rutas iniciales factibles, cumpliendo las restricciones de capacidad y ventanas de tiempo de los vehículos o clientes.
	\item \textbf{Recocido Simulado:} refina las rutas iniciales aplicando heurísticas de mejora local sin violas las restricciones mecionadas con anterioridad y aceptando soluciones subóptimas bajo un criterio probabilístico, lo que permite escapar de óptimos locales.
\end{enumerate}
...
Como primera etapa del enfoque híbrido, se emplea el Algoritmo Evolutivo Diferencial con el objetivo de calibrar los parámetros de las metaheurísticas ACO y SA de manera automatica. Esta calibración permite maximizar la calidad de las soluciones obtenidas por ACO y SA para el VRP-TW.
...
Para instancias pequeñas, se emplean rangos moderados que permiten equilibrar exploración y explotación sin incrementar el costo computacional de manera innecesaria. Como se observa en la Tabla~\ref{tab:param_pequenas}, los valores de $\alpha$ (2.0--3.5) y $\beta$ (2.5--4.0) regulan adecuadamente la influencia de las feromonas y la información heurística, respectivamente. El valor de evaporación $\rho$ (0.3--0.5) introduce una evaporación moderada que evita la convergencia prematura. El parámetro $\gamma$ (1.0--2.5) asegura el respeto de las ventanas de tiempo desde las primeras iteraciones. Con un número reducido de hormigas (3--6) y un total de 60--80 iteraciones ACO, seguido de 80--120 iteraciones SA con temperaturas iniciales entre 1200.0 y 1800.0, se logra un refinamiento eficiente en espacios de búsqueda relativamente reducidos.
...
Para instancias medianas se amplían los rangos de los parámetros con el objetivo de incrementar la capacidad de exploración en un espacio de búsqueda más complejo. Tal como se detalla en la Tabla~\ref{tab:param_medianas}, los valores de $\alpha$ (2.5--4.0) y $\beta$ (3.5--5.0) refuerzan la explotación guiada por feromonas y heurística. La evaporación $\rho$ (0.25--0.45) se mantiene en un nivel moderado para preservar buenas soluciones sin perder diversidad. Con 6--12 hormigas y 100--150 iteraciones ACO, seguidas de un refinamiento SA más exhaustivo (120--200 iteraciones y temperaturas iniciales entre 1600.0 y 2200.0), se obtiene un equilibrio adecuado entre calidad de solución y tiempo de cómputo para problemas de complejidad intermedia.
...
Para instancias grandes se requieren rangos más amplios que permitan una exploración robusta en espacios altamente dimensionales. Como se muestra en la Tabla~\ref{tab:param_grandes}, los valores elevados de $\alpha$ (3.0--5.0) y $\beta$ (4.0--6.0) intensifican la explotación de rutas prometedoras detectadas por las hormigas. Una evaporación más baja $\rho$ (0.10--0.30) conserva durante más tiempo el conocimiento acumulado, lo que resulta útil en problemas complejos. El incremento de $\gamma$ (2.0--4.0) asegura un mayor cumplimiento de las ventanas de tiempo en escenarios con rutas más extensas. Con 10--20 hormigas y 120--180 iteraciones ACO, seguido de un refinamiento SA intensivo (200--300 iteraciones y temperaturas iniciales entre 2500.0 y 4000.0), se justifica el mayor costo computacional mediante la obtención de soluciones de alta calidad.
...
La función de aptitud evalúa la calidad de cada individuo, representado por un vector de parámetros, mediante la ejecución del algoritmo híbrido ACO y SA. Su propósito es cuantificar el desempeño de cada configuración de parámetros para determinar qué tan efectiva resulta en la resolución del VRP-TW.
...
\begin{table}[H]
	...
	\begin{tabular}{|l|c|>{\raggedright\arraybackslash}p{7cm}|}
		...
		\hline
		Tamaño de población ($NP$)
		                   & $\geq 3$
		                   & Número de configuraciones de parámetros evaluadas en cada generación. Valores mayores aumentan la diversidad pero incrementan el costo computacional. Se requiere un mínimo de 3 individuos para garantizar el funcionamiento del operador de mutación.                                                                                                                                \\
		\hline
		Número de generaciones ($G$)
		                   & $\geq 1$
		                   & Número de iteraciones del algoritmo evolutivo. El valor no se ajusta automáticamente; sin embargo, se recomienda seleccionar un número adecuado en función del tamaño de la instancia. Para instancias pequeñas se sugieren valores bajos, mientras que instancias medianas y grandes requieren un mayor número de generaciones para favorecer la convergencia.                        \\
		\hline
		Factor de mutación ($F$)
		                   & 0.5
		                   & Controla la magnitud de la perturbación en el operador de mutación. Un valor de 0.5 proporciona un balance adecuado entre exploración y explotación.                                                                                                                                                                                                                                   \\
		\hline
		Tasa de cruce ($CR$)
		                   & 0.5
		                   & Probabilidad de que cada componente del vector provenga del vector mutante durante la recombinación. Un valor de 0.5 garantiza una influencia equilibrada entre ambos padres.                                                                                                                                                                                                          \\
		\hline
	\end{tabular}
...
\begin{figure}[H]
	\centering
	\begin{algorithm}[H]
		\caption{Algortimo de Evolución Diferencial para Calibración de Parámetros en el Problema de Ruteo de Vehículos con Ventanas de Tiempo}
		\label{alg:de_vrptw}
		\SetAlgoLined
	...
	\end{algorithm}
	\caption{Pseudocódigo del Evolución Diferencial para Calibración de Parámetros en el Problema de Ruteo de Vehículos con Ventanas de Tiempo.}
	\label{fig:algoritmo_de_vrptw}
\end{figure}

El pseudocódigo \ref{alg:de_vrptw} resume el funcionamiento del algoritmo de Evolución Diferencial aplicado a la calibración automática de parámetros para el VRP-TW. El proceso inicia con la generación aleatoria de una población inicial cuyos individuos son evaluados mediante la combinación ACO+SA. Posteriormente, cada generación aplica las tres etapas fundamentales de DE: mutación, recombinación y selección, produciendo nuevos candidatos que se ajustan a los rangos permitidos para cada parámetro. El mecanismo evolutivo garantiza la mejora progresiva de las soluciones, manteniendo siempre el mejor individuo encontrado durante el proceso, lo que permite identificar configuraciones de parámetros más adecuadas para las instancias evaluadas.


\subsection{Construcción de Rutas Iniciales Mediante Optimización por Colonia de Hormigas}

Una vez calibrados los parámetros mediante DE, la segunda etapa del enfoque híbrido consiste en construir soluciones factibles para el VRP-TW utilizando Optimización por Colonia de Hormigas. Este algoritmo bio-inspirado simula el comportamiento de las hormigas reales al buscar alimento, utilizando rastros de feromonas para encontrar rutas eficientes que minimicen la distancia total recorrida mientras respetan las restricciones de capacidad de los vehículos y las ventanas de tiempo de los clientes.

\subsubsection{Fundamentos del Algoritmo Optimización por Colonia de Hormigas}
...
\begin{figure}[H]
	\centering
	\begin{algorithm}[H]
		\caption{Cálculo de Clientes Factibles en Optimización por Colonia de Hormigas.}
	...
	\end{algorithm}
	\caption{Pseudocódigo para Cálculo de Clientes Factibles en Optimización por Colonia de Hormigas.}
...
\begin{figure}[H]
	...
	\end{algorithm}
	\caption{Pseudocódigo de Construcción de Solución por una Hormiga.}
...
\begin{figure}[H]
	\centering
	\begin{algorithm}[H]
		\caption{Evaluación de la Función Objetivo en Optimización por Colonia de Hormigas}
...
\subsubsection*{Evaporación de Feromonas}
...
\subsubsection*{Depósito de Feromonas}
\raggedright
...
\subsubsection*{Refuerzo Elitista}
...
\begin{figure}[H]
	...
	\begin{algorithm}[H]
		...
		\textbf{Evaporación:}\\
		\textbf{para} $i=0$ \textbf{hasta} $n-1$ \textbf{hacer}\\
		\hspace{1em} \textbf{para} $j=0$ \textbf{hasta} $n-1$ \textbf{hacer}\\
		\hspace{2em} \textbf{si} $i \neq j$ \textbf{entonces} $\tau_{ij} \gets (1 - \rho) \cdot \tau_{ij}$ \textbf{fin si}\\
		\hspace{1em} \textbf{fin para}\\
		\textbf{fin para}\\

		\textbf{Depósito de feromonas:}\\
		\textbf{para cada} hormiga $k$ en Elite \textbf{hacer}\\
		\hspace{1em} $\Delta\tau \gets 1 / L_k$\\
		\hspace{1em} \textbf{para cada} arista $(i,j)$ en la ruta de $k$ \textbf{hacer}\\
		\hspace{2em} $\tau_{ij} \gets \tau_{ij} + \Delta\tau$; $\tau_{ji} \gets \tau_{ji} + \Delta\tau$\\
		\hspace{1em} \textbf{fin para}\\
		\textbf{fin para}\\

		\textbf{Refuerzo elitista:}\\
		$\Delta\tau_{best} \gets 2 / L_{best}$\\
		\textbf{para cada} arista $(i,j)$ en MejorRuta \textbf{hacer}\\
		\hspace{1em} $\tau_{ij} \gets \tau_{ij} + \Delta\tau_{best}$; $\tau_{ji} \gets \tau_{ji} + \Delta\tau_{best}$\\
		\textbf{fin para}\\
...
\subsubsection{Pseudocódigo del Algortimo Optimización por Colonia de Hormigas Integrado con Recocido Simulado}
...
\begin{figure}[H]
	\centering
	\begin{algorithm}[H]
		\caption{Optimización por Colonia de Hormigas con Recocido Simulado para el Problema de Ruteo de Vehículos con Ven-
			tanas de Tiempo}
		\label{alg:aco_vrptw}
		\SetAlgoLined
		\SetAlFnt{\footnotesize}
	...
	\end{algorithm}
	\caption{Pseudocódigo de la Optimización por Colonia de Hormigas con Recocido Simulado para Problema de Ruteo de Vehículos con Ventanas de Tiempo.}
...
Para explorar el espacio de soluciones, SA implementa un conjunto de ocho operadores de vecindad especializados que modifican las rutas de manera controlada. La selección de operadores se realiza de forma adaptativa según el tamaño de la instancia y el número de vehículos en la solución actual, ajustando dinámicamente la exploración del espacio de búsqueda.

Los operadores se dividen en dos categorías: operadores intra-ruta, que modifican únicamente la secuencia de visitas dentro de cada ruta, y operadores inter-ruta, que redistribuyen clientes entre diferentes vehículos.

\subsubsection*{Operadores Intra-Ruta}

Los operadores intra-ruta optimizan la secuencia de visitas dentro de una misma ruta sin transferir clientes entre vehículos. Son efectivos para eliminar cruces geométricos, reducir distancias locales y mejorar el cumplimiento de ventanas de tiempo. Su principal ventaja es que mantienen constante la carga del vehículo, reduciendo el riesgo de violar restricciones de capacidad.
...
Los operadores inter-ruta transfieren clientes o segmentos entre diferentes vehículos, realizando modificaciones más disruptivas. Son esenciales para explorar regiones distantes del espacio de soluciones y tienen potencial para reducir el número total de vehículos necesarios.

Debido a que modifican la asignación de clientes a vehículos, requieren verificación rigurosa de las restricciones de capacidad y ventanas de tiempo en ambas rutas afectadas.
	...
	\item \textbf{Swap Inter-Ruta:} Intercambia un cliente de una ruta con un cliente de otra ruta.
	      ...
	      \begin{figure}[H]
		      ...
		      \begin{algorithm}[H]
			      ...
			      \textbf{si} verificar\_restricciones(ambos) \textbf{entonces} retornar $s'$ \textbf{si no} revertir.\\
		      \end{algorithm}
...
\subsubsection*{Estrategia Adaptativa de Selección}

La probabilidad de aplicar cada operador se ajusta dinámicamente considerando el tamaño de la instancia y el número de vehículos en la solución actual. Esta adaptación permite enfatizar diferentes estrategias de búsqueda según el contexto específico.

En instancias pequeñas ($n \leq 26$ clientes) con múltiples vehículos, se priorizan operadores inter-ruta intensivos: cross-exchange (25\%), relocate-chain (20\%) y swap inter (20\%) para consolidar rutas. Con un solo vehículo, se enfatiza la optimización intra-ruta: relocate-chain (30\%), 2.5-opt (25\%) y reinserción (20\%).

Para instancias medianas ($26 < n \leq 51$ clientes), la estrategia busca balance. Con múltiples vehículos se mantienen activos operadores inter-ruta (cross-exchange 30\%, relocate-chain 25\%, swap inter 20\%) e intra-ruta. Con un solo vehículo aumenta el peso en relocate-chain (35\%), 2.5-opt (30\%) y reinserción (20\%).

En instancias grandes ($n > 51$ clientes), la complejidad combinatoria requiere operadores más agresivos. Con múltiples vehículos dominan relocate-chain (35\%) y cross-exchange (30\%), complementados con swap inter (20\%). Con un solo vehículo se aplica fuerte énfasis en relocate-chain (40\%) y 2.5-opt (35\%).
...
Dado que los operadores de vecindad pueden generar soluciones infactibles que violan restricciones de capacidad o ventanas de tiempo, se implementan mecanismos de validación rigurosos para asegurar que cada ruta cumpla con las restricciones operativas del VRP-TW.

\begin{figure}[H]
	\centering
	\begin{algorithm}[H]
		\caption{Verificación de Restricciones y Factibilidad de Ruta}
		...
		\textbf{Inicialización:} $t \gets 0$, $q \gets 0$, clienteAnterior $\gets$ NULL\\
		...
		\hspace{3em} $t \gets t + s_{\text{clienteAnterior}}$\\
		\hspace{2em} \textbf{fin si}\\
		\hspace{2em} $t \gets t + D_{\text{clienteAnterior}, \text{cliente}} / v.\text{velocidad}$\\
		\hspace{2em} \textbf{si} $t < e_{\text{cliente}}$ \textbf{entonces} $t \gets e_{\text{cliente}}$ \textbf{fin si}\\
		\hspace{1em} \textbf{si no}\\
		\hspace{2em} \textbf{si} cliente $\neq 0$ \textbf{entonces}\\
		\hspace{3em} $t \gets D_{0, \text{cliente}} / v.\text{velocidad}$\\
		\hspace{3em} \textbf{si} $t < e_{\text{cliente}}$ \textbf{entonces} $t \gets e_{\text{cliente}}$ \textbf{fin si}\\
		\hspace{2em} \textbf{fin si}\\
		\hspace{1em} \textbf{fin si}\\
		\hspace{1em} \textbf{si} cliente $\neq 0$ \textbf{entonces}\\
		\hspace{2em} $q \gets q + d_{\text{cliente}}$\\
		\hspace{2em} \textbf{si} $q > v.Q$ \textbf{entonces} retornar false \textbf{fin si}\\
		\hspace{2em} \textbf{si} $t > l_{\text{cliente}}$ \textbf{entonces} retornar false \textbf{fin si}\\
		\hspace{1em} \textbf{fin si}\\
		\hspace{1em} clienteAnterior $\gets$ cliente\\
		\textbf{fin para}\\

		\textbf{si} clienteAnterior $\neq 0$ \textbf{entonces}\\
		\hspace{1em} $t \gets t + s_{\text{clienteAnterior}} + D_{\text{clienteAnterior}, 0} / v.\text{velocidad}$\\
		\hspace{1em} \textbf{si} $t > l_0$ \textbf{entonces} retornar false \textbf{fin si}\\
		\textbf{fin si}\\

		retornar true\\

	\end{algorithm}
	\caption{Pseudocódigo de Verificación de Restricciones y Factibilidad de Ruta.}
	\label{fig:verificacion_restricciones}
\end{figure}


El pseudocódigo \ref{fig:verificacion_restricciones} ejecuta una verificación completa de factibilidad sobre cada vehículo. El procedimiento evalúa secuencialmente los tiempos de viaje, los tiempos de servicio y las esperas necesarias, garantizando que cada cliente sea atendido dentro de su ventana de tiempo. Además, acumula la demanda total para verificar que no se exceda la capacidad del vehículo y comprueba que el retorno al depósito pueda realizarse antes del cierre de su ventana temporal. Esta validación asegura que cualquier ruta generada por los operadores de vecindad sea descartada de inmediato si incurre en violaciones, manteniendo así la coherencia y factibilidad del conjunto completo de rutas.
...
Los operadores inter-ruta pueden dejar ciertos vehículos sin clientes. Debido a esto, se implementó un procedimiento de eliminación de vehículos vacíos con el fin de mantener una flota compacta y consistente.
...
\begin{figure}[H]
	...
	\end{algorithm}
	\caption{Pseudocódigo de Eliminación de Vehículos Vacíos.}
	\label{fig:limpieza_vehiculos}
\end{figure}

El pseudocódigo \ref{fig:limpieza_vehiculos} describe el procedimiento encargado de eliminar de la flota aquellos vehículos que han quedado sin clientes después de aplicar operadores inter-ruta. Este proceso se ejecuta periódicamente cada 20 iteraciones y nuevamente al finalizar el algoritmo SA, con el fin de asegurar que la solución final sea compacta y no contenga vehículos innecesarios. La limpieza permite mantener la estructura de la flota consistente, reducir el número total de vehículos utilizados y garantizar que el costo reportado refleje únicamente rutas activas y realmente empleadas.

La limpieza de vehículos vacíos se realiza en los siguientes momentos:
\begin{itemize}
	\item Después de aplicar los operadores \textit{swap\_inter}, \textit{reinserción}, \textit{cross-exchange} y \textit{relocate-chain}.
	\item De manera periódica, cada 20 iteraciones.
	\item Al finalizar la ejecución completa del algoritmo SA.
\end{itemize}


Cuando se eliminan vehículos, es necesario re-evaluar la función objetivo completa para reflejar la nueva estructura de la solución, como se muestra en el pseudocódigo.

\begin{figure}[H]
	\centering
	\begin{algorithm}[H]
		\caption{Evaluación de la Función Objetivo en Recocido Simulado}
	...
	\end{algorithm}
	\caption{Pseudocódigo de Evaluación de la Función Objetivo en Recocido Simulado.}
	\label{fig:evaluacion_fitness_sa}
\end{figure}

El pseudocódigo \ref{fig:evaluacion_fitness_sa} recorre todos los vehículos de la flota y acumula las distancias entre clientes consecutivos en cada ruta. Este proceso considera completamente la estructura de cada vehículo, incluyendo los desplazamientos desde y hacia el depósito cuando corresponda. Con ello se actualiza el fitness global de la solución tras cualquier modificación en la flota o en las rutas.

\subsubsection{Pseudocódigo del Algoritmo de Recocido Simulado}

El proceso de Recocido Simulado (SA) se utiliza como un mecanismo de mejora local para refinar las soluciones generadas inicialmente por el algoritmo ACO. Su objetivo es explorar vecindarios diversos mediante un esquema probabilístico que permite aceptar soluciones de peor calidad de manera controlada, con el fin de escapar de óptimos locales, como se muestra en el pseudocódigo.

\begin{figure}[H]
	\centering
	\begin{algorithm}[H]
		\caption{Recocido Simulado para Refinamiento de Rutas el Problema de Ruteo de Vehículos con Ventanas de Tiempo}
		\label{alg:sa_vrptw}
		\SetAlgoLined
		\SetAlFnt{\scriptsize}
		\setlength{\algomargin}{0.5em}
		...
		\textbf{Inicialización:} $s_{\text{vecina}} \gets$ copiar($s_0$), $s_{\text{actual}} \gets$ copiar($s_0$), $s^* \gets$ copiar($s_0$), $T \gets T_0$\\

		\textbf{mientras} $T > T_f$ y $T > 10^{-10}$ \textbf{hacer}\\
		\hspace{1em} \textbf{para} $iter=1$ \textbf{hasta} $n_{\text{iter}}$ \textbf{hacer}\\
		\hspace{2em} Liberar($s_{\text{vecina}}.$flota); $s_{\text{vecina}} \gets$ copiar($s_{\text{actual}}$)\\
		\hspace{2em} $r \gets$ aleatorio$[0,1]$; operador $\gets$ SeleccionarOperador($r$, instancia.tamaño, $s_{\text{vecina}}.$vehículos)\\
		\hspace{2em} aceptado $\gets$ AplicarOperador(operador, $s_{\text{vecina}}$)\\
		\hspace{2em} \textbf{si} no aceptado \textbf{entonces} continuar \textbf{fin si}\\
		...
		\hspace{3em} \textbf{si} $v_{\text{antes}} \neq s_{\text{vecina}}.$vehiculos\_necesarios \textbf{entonces} EvaluarFitness($s_{\text{vecina}}$) \textbf{fin si}\\
		\hspace{2em} \textbf{fin si}\\

		\hspace{2em} \textbf{si} $iter \mod 20 = 0$ \textbf{entonces} EliminarVehiculosVacios($s_{\text{vecina}}.$flota) \textbf{fin si}\\

		\hspace{2em} EvaluarFitness($s_{\text{vecina}}$); $\Delta f \gets s_{\text{vecina}}.$fitness $- s_{\text{actual}}.$fitness\\

		\hspace{2em} aceptar $\gets$ false\\
		\hspace{2em} \textbf{si} $\Delta f < -10^{-6}$ \textbf{entonces} aceptar $\gets$ true\\
		\hspace{2em} \textbf{si no, si} $\Delta f > 10^{-6}$ y $T > 10^{-10}$ \textbf{entonces}\\
		\hspace{3em} $p \gets e^{-\Delta f / T}$; \textbf{si} aleatorio$[0,1] < p$ \textbf{entonces} aceptar $\gets$ true \textbf{fin si}\\
		\hspace{2em} \textbf{fin si}\\

		\hspace{2em} \textbf{si} aceptar \textbf{entonces}\\
		\hspace{3em} Liberar($s_{\text{actual}}.$flota); $s_{\text{actual}} \gets$ copiar($s_{\text{vecina}}$)\\
		\hspace{3em} \textbf{si} $s_{\text{vecina}}.$fitness $< s^*.$fitness $- 10^{-6}$ \textbf{entonces}\\
		\hspace{4em} Liberar($s^*.$flota); $s^* \gets$ copiar($s_{\text{vecina}}$)\\
		\hspace{3em} \textbf{fin si}\\
		\hspace{2em} \textbf{fin si}\\
		\hspace{1em} \textbf{fin para}\\

		\hspace{1em} $T \gets \alpha_{\text{cool}} \cdot T$; \textbf{si} $T < 10^{-10}$ \textbf{entonces} romper \textbf{fin si}\\
		\textbf{fin mientras}\\

		\textbf{si} $s^*.$vehiculos\_necesarios $> 1$ \textbf{entonces}\\
		\hspace{1em} $v_{\text{antes}} \gets s^*.$vehiculos\_necesarios; $s^*.$vehiculos\_necesarios $\gets$ EliminarVehiculosVacios($s^*.$flota)\\
		\hspace{1em} \textbf{si} $v_{\text{antes}} \neq s^*.$vehiculos\_necesarios \textbf{entonces} EvaluarFitness($s^*$) \textbf{fin si}\\
		\textbf{fin si}\\

		Liberar memoria temporal; \textbf{retornar} $s^*$

	\end{algorithm}
	\caption{Pseudocódigo de Recocido Simulado para Refinamiento de Rutas Problema de Ruteo de Vehículos con Ventanas de Tiempo.}
	\label{fig:algoritmo_sa_principal}
\end{figure}

El pseudocódigo~\ref{alg:sa_vrptw} describe el procedimiento completo del algoritmo de Recocido Simulado aplicado al refinamiento de rutas, incluyendo la selección adaptativa de operadores de vecindad en función de las características de la solución actual, el esquema de enfriamiento gradual de la temperatura, el criterio de aceptación probabilística de soluciones de peor calidad y la actualización de la mejor solución encontrada. Asimismo, se incorpora la gestión explícita de memoria para las estructuras de rutas y flotas, garantizando la consistencia y eficiencia durante el proceso iterativo de búsqueda.

\chapter{Resultados}
\label{cap:resultados}

Se presentan los resultados obtenidos al aplicar el algoritmo híbrido DE+ACO+SA al VRP-TW. Se realizaron pruebas con instancias de 25, 50 y 100 clientes, ejecutando cada instancia 15 veces para evaluar la consistencia y robustez del algoritmo propuesto.

\section{Resultados con 25 Clientes}

Las instancias con 25 clientes representan problemas de tamaño pequeño que permiten validar el correcto funcionamiento del algoritmo. La Tabla~\ref{tab:results_25} muestra los resultados obtenidos.

\begin{table}[H]
	\centering
	\caption{Resultados para Instancias de 25 clientes}
	\label{tab:results_25}
	\begin{tabular}{|l|c|c|c|c|c|}
		\hline
		\textbf{Instancia} & \textbf{Mejor Reportado} & \textbf{Mejor} & \textbf{Peor} & \textbf{Promedio} & \textbf{Desv. Est.} \\
		\hline
		C101               & 191.3                    & 191.3          & 191.3         & 191.3             & 0.00                \\
		C102               & 190.3                    & 190.3          & 190.3         & 190.3             & 0.00                \\
		C103               & 190.3                    & 190.3          & 190.3         & 190.3             & 0.00                \\
		C201               & 214.7                    & 214.7          & 214.7         & 214.7             & 0.00                \\
		C202               & 214.7                    & 214.7          & 214.7         & 214.7             & 0.00                \\
		\hline
	\end{tabular}
\end{table}

El algoritmo híbrido demostró una excelente consistencia en estas instancias, alcanzando la solución óptima en todas las ejecuciones. La desviación estándar de 0.00 indica que el algoritmo convergió al mismo resultado en las 15 pruebas, lo cual refleja la efectividad de la combinación DE+ACO+SA para problemas de esta escala.

Se muestran las rutas generadas para todas las instancias de 25 clientes. Dado que todas las ejecuciones convergieron al mismo resultado, las rutas de mejor y peor caso son idénticas en todas las instancias.

\begin{figure}[H]
	\centering
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c101_25_mejor.png}
		\caption{Mejor solución C101 (191.3)}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c101_25_peor.png}
		\caption{Peor solución C101 (191.3)}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c102_25_mejor.png}
		\caption{Mejor solución C102 (190.3)}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c102_25_peor.png}
		\caption{Peor solución C102 (190.3)}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c103_25_mejor.png}
		\caption{Mejor solución C103 (190.3)}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c103_25_peor.png}
		\caption{Peor solución C103 (190.3)}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c201_25_mejor.png}
		\caption{Mejor solución C201 (214.7)}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c201_25_peor.png}
		\caption{Peor solución C201 (214.7)}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c202_25_mejor.png}
		\caption{Mejor solución C202 (214.7)}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c202_25_peor.png}
		\caption{Peor solución C202 (214.7)}
	\end{minipage}
\end{figure}

\section{Resultados con 50 Clientes}

Las instancias de 50 clientes presentan mayor complejidad debido al incremento en el espacio de búsqueda. Los resultados se presentan en la Tabla~\ref{tab:results_50}.

\begin{table}[H]
	\centering
	\caption{Resultados para Instancias de 50 clientes}
	\label{tab:results_50}
	\begin{tabular}{|l|c|c|c|c|c|}
		\hline
		\textbf{Instancia} & \textbf{Mejor Reportado} & \textbf{Mejor} & \textbf{Peor} & \textbf{Promedio} & \textbf{Desv. Est.} \\
		\hline
		C104               & 359.0                    & 358.0          & 360.5         & 359.4             & 1.04                \\
		C105               & 362.4                    & 362.4          & 362.4         & 362.4             & 0.00                \\
		C106               & 362.4                    & 362.4          & 362.4         & 362.4             & 0.00                \\
		C203               & 359.8                    & 359.8          & 364.8         & 361.8             & 1.77                \\
		C204               & 353.4                    & 350.1          & 359.6         & 352.9             & 3.06                \\
		\hline
	\end{tabular}
\end{table}

En este conjunto, el algoritmo mantuvo un buen desempeño aunque con mayor variabilidad. Las instancias C105 y C106 mantuvieron consistencia perfecta, mientras que C104, C203 y C204 mostraron variaciones moderadas. La instancia C204 presentó la mayor desviación estándar (3.06), con una diferencia de 9.5 unidades entre la mejor y peor solución, sugiriendo que el paisaje de búsqueda de esta instancia presenta mayor complejidad. El componente de Simulated Annealing ayudó a escapar de óptimos locales, aunque la naturaleza estocástica del algoritmo generó cierta dispersión en los resultados.

Se ilustran las rutas obtenidas para las instancias de 50 clientes. En este conjunto se observan diferencias más notables entre mejor y peor caso, especialmente en C104, C203 y C204, reflejando la mayor complejidad de estas instancias.

\begin{figure}[H]
	\centering
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c104_50_mejor.png}
		\caption{Mejor solución C104 (358.0)}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c104_50_peor.png}
		\caption{Peor solución C104 (360.5)}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c105_50_mejor.png}
		\caption{Mejor solución C105 (362.4)}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c105_50_peor.png}
		\caption{Peor solución C105 (362.4)}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c106_50_mejor.png}
		\caption{Mejor solución C106 (362.4)}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c106_50_peor.png}
		\caption{Peor solución C106 (362.4)}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c203_50_mejor.png}
		\caption{Mejor solución C203 (359.8)}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c203_50_peor.png}
		\caption{Peor solución C203 (364.8)}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c204_50_mejor.png}
		\caption{Mejor solución C204 (350.1)}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c204_50_peor.png}
		\caption{Peor solución C204 (359.6)}
	\end{minipage}
\end{figure}

\section{Resultados con 100 Clientes}

Las instancias de 100 clientes representan el mayor desafío, con un espacio de soluciones significativamente más amplio. La Tabla~\ref{tab:results_100} resume los resultados.

\begin{table}[H]
	\centering
	\caption{Resultados para Instancias de 100 clientes}
	\label{tab:results_100}
	\begin{tabular}{|l|c|c|c|c|c|}
		\hline
		\textbf{Instancia} & \textbf{Mejor Reportado} & \textbf{Mejor} & \textbf{Peor} & \textbf{Promedio} & \textbf{Desv. Est.} \\
		\hline
		C101               & 827.3                    & 827.3          & 827.3         & 827.3             & 0.00                \\
		C107               & 827.3                    & 827.3          & 837.1         & 830.2             & 3.89                \\
		C108               & 827.3                    & 827.3          & 851.4         & 832.5             & 8.04                \\
		\hline
	\end{tabular}
\end{table}

En las instancias de 100 clientes, se observa un comportamiento diferenciado. C101 mantuvo consistencia perfecta, logrando la solución óptima en todas las ejecuciones. Sin embargo, C107 y especialmente C108 mostraron mayor variabilidad. La instancia C108 presentó la mayor desviación estándar (8.04) de todo el conjunto experimental, con una diferencia de 24.1 unidades entre mejor y peor caso. Esto indica que la complejidad del problema escala de manera no lineal, y que la exploración del espacio de búsqueda requiere más iteraciones o ajustes paramétricos para instancias de mayor tamaño.

\begin{figure}[H]
	\centering
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c101_100_mejor.png}
		\caption{Mejor solución C101 (827.3)}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c101_100_peor.png}
		\caption{Peor solución C101 (827.3)}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c107_100_mejor.png}
		\caption{Mejor solución C107 (827.3)}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c107_100_peor.png}
		\caption{Peor solución C107 (837.1)}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c108_100_mejor.png}
		\caption{Mejor solución C108 (827.3)}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Imagenes/c108_100_peor.png}
		\caption{Peor solución C108 (851.4)}
	\end{minipage}
\end{figure}

El algoritmo híbrido demostró un desempeño robusto en las instancias evaluadas. En instancias pequeñas de 25 clientes, el algoritmo alcanzó consistentemente soluciones óptimas con desviación estándar nula en todos los casos, lo que valida la correcta integración de los tres componentes del enfoque híbrido. Para las instancias de escala media con 50 clientes, se observó una variabilidad controlada, con desviaciones estándar que no superaron las 3.06 unidades, indicando que el algoritmo mantiene una buena calidad de solución aun con el incremento de la complejidad del problema. 

Sin embargo, en las instancias grandes de 100 clientes se evidenció una mayor dispersión en los resultados, lo que sugiere desafíos en términos de escalabilidad, particularmente en instancias con características específicas como C107 y C108, donde la desviación estándar alcanzó valores de hasta 8.04. La combinación del algoritmo de Evolución Diferencial para la diversificación, la Optimización por Colonia de Hormigas para la construcción de soluciones y el Recocido Simulado para el refinamiento local resultó efectiva; no obstante, los resultados sugieren que ajustes paramétricos específicos podrían mejorar el desempeño en instancias de gran escala. En general, el algoritmo logró igualar las mejores soluciones reportadas en la literatura en la mayoría de los casos.

\chapter{Conclusiones y Trabajo Futuro}
\label{cap:conclusiones}

El presente estudio propuso un enfoque híbrido basado en la combinación de Evolución Diferencial, Optimización por Colonia de Hormigas y Recocido Simulado para resolver el Problema de Ruteo de Vehículos con Ventanas de Tiempo. Los resultados obtenidos demostraron que la integración de estas tres técnicas permite construir soluciones factibles de manera consistente y refinarlas posteriormente para reducir la distancia total recorrida y el número de vehículos empleados. El método híbrido mostró ser competitivo al compararse con valores de referencia reportados en la literatura para las instancias de Solomon.

El objetivo principal del trabajo fue diseñar y evaluar un método híbrido capaz de mejorar el desempeño de técnicas metaheurísticas tradicionales aplicadas al VRP-TW. Este objetivo se cumplió satisfactoriamente al integrar DE para la calibración automática de parámetros, ACO para la construcción inicial de rutas y SA como mecanismo de mejora local. Cada componente desempeñó el rol esperado dentro del enfoque híbrido, contribuyendo a obtener soluciones de mayor calidad y reduciendo el esfuerzo manual asociado a la calibración de parámetros.

Entre las líneas futuras de investigación se propone explorar versiones paralelas del método híbrido que permitan reducir los tiempos de ejecución sin comprometer la calidad de las soluciones. También resulta prometedor incorporar nuevas restricciones del mundo real, como múltiples depósitos, flotas heterogéneas o prioridades diferenciadas entre clientes. Otra línea relevante consiste en extender el análisis a otros conjuntos de instancias más desafiantes, así como evaluar el desempeño del método en problemas de mayor escala que reflejen con mayor fidelidad las complejidades de la logística moderna.

Finalmente, se confirma que el objetivo del trabajo fue alcanzado, dado que el método híbrido desarrollado logró generar soluciones competitivas frente a las técnicas reportadas en la literatura. Esto demuestra que la integración de DE, ACO y SA constituye una alternativa sólida, viable y eficaz para abordar el VRP-TW.
