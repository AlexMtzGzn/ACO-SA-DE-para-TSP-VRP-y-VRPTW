...
do
    ...
    while IFS= read -r line; do
        # Verificar si es una línea con "READY TIME", "DUE DATE", "SERVICE" en el encabezado
        if [[ $line == *"READY TIME"* && $line == *"DUE DATE"* && $line == *"SERVICE"* ]]; then
            # Eliminar esas columnas del encabezado
            header=$(echo "$line" | sed -E 's/READY TIME\s+DUE DATE\s+SERVICE\s+TIME\s*//g')
            echo "$header" >> "$temp_file"
        # Verificar si es una línea numérica de datos (busca patrones de números)
        elif [[ $line =~ [0-9]+[[:space:]]+[0-9]+[[:space:]]+[0-9]+ ]]; then
            # Extraer los primeros 4 campos (CUST NO, XCOORD, YCOORD, DEMAND)
            customer=$(echo "$line" | awk '{printf "%-10s %-10s %-10s %s", $1, $2, $3, $4}')
    ...
    echo "Archivo $archivo recortado correctamente (eliminadas las columnas de READY TIME, DUE DATE y SERVICE TIME)."
done